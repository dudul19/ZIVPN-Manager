...
# Banner Checking IP
clear
echo -e "${YELLLOW}Wait.. Checking Your System & IP Address..${RESET}"
sleep 5
clear

# Banner Welcome
echo -e "${GREEN}======================================${RESET}"
echo -e "${YELLLOW}Welcome to ZiVPN UDP Manager Installer${RESET}"
echo -e "${GREEN}======================================${RESET}"
echo -e "${BLUE}> IP Address : ${RESET}${RED}${IP}${RESET}"
echo -e "${BLUE}> System OS  : ${RESET}${RED}${OS}${RESET}"
echo -e "${BLUE}> Installer  : ${RESET}${RED}${VERSION}${RESET}"
echo -e "${BLUE}> Developer  : ${RESET}${RED}${DEV}${RESET}"
echo -e "${BLUE}> Recode     : ${RESET}${RED}${AUTH}${RESET}"
echo -e "${GREEN}======================================${RESET}"
echo ""
echo -e "${BLUE}> Press ${RED}[ENTER] ${BLUE}To Start Installation...${RESET}"
read
echo ""
clear
...
function install_domain() {
    ...
    echo -e "${CYAN}           Domain Configuration        ${RESET}"
    echo -e "${GREEN}======================================${RESET}"
    echo -e "${CYAN}  1) Use Own Domain"
    echo -e "${CYAN}  2) Use Domain Script"
    echo -e "${GREEN}======================================${RESET}"
    read -p "    Choose [1/2] : " host
    echo ""
    if [[ $host == "1" ]]; then
      echo -e "${CYAN}  > Enter Your Domain: ${RESET}"
      read -p "Subdomain: " domain
      echo $domain > /etc/zivpn/domain
      echo ""
    elif [[ $host == "2" ]]; then
      wget -q https://raw.githubusercontent.com/dudul19/zivpn-manager/cf.sh && chmod +x cf.sh && ./cf.sh
      rm -f /root/cf.sh
      clear
    else
...
run_silent "Configuring" "wget -q https://raw.githubusercontent.com/dudul19/zivpn-manager/main/config.json -O /etc/zivpn/config.json"
...
run_silent "Setting up API" "wget -q https://raw.githubusercontent.com/dudul19/zivpn-manager/main/zivpn-api.go -O /etc/zivpn/api/zivpn-api.go && wget -q https://raw.githubusercontent.com/AutoFTbot/ZiVPN/main/go.mod -O /etc/zivpn/api/go.mod"
...
echo ""
echo -e "${GREEN}======================================${RESET}"
echo -e "${CYAN}        Telegram Bot Configuration     ${RESET}"
echo -e "${GREEN}======================================${RESET}"
...
if [[ -n "$bot_token" ]] && [[ -n "$admin_id" ]]; then
  echo ""
  echo -e "${GREEN}======================================${RESET}"
  echo -e "${CYAN}             Select Bot Type           ${RESET}"
  echo -e "${GREEN}======================================${RESET}"
  ...
  run_silent "Downloading Bot" "wget -q https://raw.githubusercontent.com/dudul19/zivpn-manager/main/$bot_file -O /etc/zivpn/api/$bot_file"
...
# Install menu
wget -q https://raw.githubusercontent.com/dudul19/zivpn-manager/main/menu >/dev/null 2>&1
chmod +x menu
mv menu /usr/local/sbin
rm -rf menu

# Set Menu
sed -i '/\/usr\/local\/sbin\/menu/d' ~/.bashrc
cat <<EOF >> ~/.bashrc
# Auto-Launch ZiVPN Menu
...
echo -e "  > Developer : ${CYAN}${DEV}${RESET}"
echo -e "  > Recode    : ${CYAN}${AUTH}${RESET}"
echo -e "${GREEN}======================================${RESET}"
echo ""
echo -ne "  ${GREEN} Server Will Reboot in 5 Seconds... ${RESET}"
sleep 1
echo -ne "${GREEN}4... ${RESET}"
sleep 1
echo -ne "${RED}3... ${RESET}"
sleep 1
